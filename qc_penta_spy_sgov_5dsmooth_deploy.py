"""
QuantConnect Deployment: Penta 5d-Smoothed (SPY / SGOV)
========================================================
Binary Penta rotation with 5-day SMA smoothing:

  Risk on (smoothed Penta > 0.5):  100% SPY
  Risk off:                         100% SGOV

Same signal logic as the SPMO variant for apples-to-apples comparison.

Usage:
    python qc_penta_spy_sgov_5dsmooth_deploy.py
"""
...
class PentaSPYSGOV(QCAlgorithm):
    """
    Penta 5d-smooth: SPY risk-on / SGOV risk-off.
    Same signal as SPMO variant for comparison.
    """
    ...
    def initialize(self):
        ...
        self.spy_trade = self.add_equity("SPY", Resolution.DAILY).symbol
        self.sgov = self.add_equity("SGOV", Resolution.DAILY).symbol

        self.spy_sym = self.spy_trade
        ...
        self.securities[self.spy_trade].set_slippage_model(slippage)
    ...
    def _get_penta_and_regime(self):
        ...
        return ("RISK_ON" if penta_on else "RISK_OFF"), ("SPY" if penta_on else "SGOV"), smoothed
    ...
    def _daily_check(self):
        ...
        self.liquidate()
        if target == "SPY":
            self.set_holdings(self.spy_trade, 1.0)
        elif target == "SGOV":
            self.set_holdings(self.sgov, 1.0)

    def on_end_of_algorithm(self):
        years = (self.end_date - self.start_date).days / 365.25
        self.debug("=" * 50)
        self.debug("PENTA 5d-SMOOTH: SPY / SGOV SUMMARY")
        self.debug("=" * 50)
        self.debug(f"Signal: 5d SMA of binary Penta > 0.5")
        self.debug(f"Risk-on:  100% SPY")
        self.debug(f"Risk-off: 100% SGOV")
...
def main():
    print("=" * 70)
    print("QUANTCONNECT: Penta 5d-Smooth (SPY / SGOV)")
    print("=" * 70)
    ...
    project_name = f"Penta5dSmooth_SPY_SGOV_{ts}"
    ...
    print("\n5. Starting backtest...")
    bt_name = f"Penta5dSmooth_SPY_SGOV_{ts}"
    ...
    print("\n" + "=" * 70)
    print("PENTA 5d-SMOOTH SPY / SGOV RESULTS")
    print("=" * 70)
    print(f"  Project:  {project_name}")
    print(f"  Backtest: {bt_name}")
    print("  Signal:     5d SMA of binary Penta > 0.5 (smoothed)")
    print("  Risk on:    100% SPY")
    print("  Risk off:   100% SGOV")
    ...
    out = os.path.join(SCRIPT_DIR, "qc_penta_5dsmooth_spy_sgov_results.json")
