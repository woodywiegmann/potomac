...
================================
Creates a project on QuantConnect, uploads the CRTOX TLH algorithm,
compiles it, runs a backtest, and retrieves results.
...
def main():
    print("=" * 70)
    print("QUANTCONNECT DEPLOYMENT: CRTOX Momentum + TLH")
    print("=" * 70)
    ...
    print("\n2. Creating project...")
    project_name = f"CRTOX_Momentum_TLH_{int(time_mod.time())}"
    r = api("/projects/create", {"name": project_name, "language": "Py"})
    if not r.get("success"):
    ...
    # Step 3: Upload algorithm file (update default main.py)
    print("\n3. Uploading algorithm code...")
    r = api("/files/update", {
    ...
    print("\n5. Starting backtest...")
    backtest_name = f"CRTOX_TLH_Backtest_{int(time_mod.time())}"
    ...
    print(f"   Backtest started: {backtest_name} (ID: {backtest_id})")

    # Step 6: Poll for backtest completion
    ...
    for attempt in range(120):
        ...
        print(f"   Progress: {progress:.0%} | Completed: {completed}", end="\r")
    ...
    print(f"  Backtest: {backtest_name}")
    print(f"  Period: {bt.get('created', 'N/A')}")

    print("\n  KEY METRICS:")
    for key in ["Total Return", "Compounding Annual Return", "Drawdown",
                 "Sharpe Ratio", "Sortino Ratio", "Calmar Ratio",
                 "Alpha", "Beta", "Total Fees",
                 "Estimated Strategy Capacity", "Portfolio Turnover",
    ...
    out_path = os.path.join(SCRIPT_DIR, "qc_backtest_results.json")
    with open(out_path, "w") as f:
        json.dump(r, f, indent=2, default=str)
    print(f"\n  Full results saved to: {out_path}")

    # Check logs for TLH summary
    print("\n  ALGORITHM LOGS (last 20):")
    logs = bt.get("logs", "")
    if logs:
    ...
    print("\n" + "=" * 70)
    print(f"View in browser: https://www.quantconnect.com/terminal/{project_id}#open/{backtest_id}")
    print("=" * 70)
